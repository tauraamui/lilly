...
lib__clipboard__CClipboardContent* clipboard_get_content(void) {
	lib__clipboard__CClipboardContent* clipboard_content = NULL;
	@autoreleasepool {
		NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
		...
		if (archivedData) {
			...
			if (contentDict && !error && [contentDict isKindOfClass:[NSDictionary class]]) {
				...
				if (text && t_type) {
					clipboard_content = malloc(sizeof(lib__clipboard__CClipboardContent));
